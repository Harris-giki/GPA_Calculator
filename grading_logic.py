# -*- coding: utf-8 -*-
"""CombineAllFunctioning.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1h2W42yBEiFh0cLZ0AYBSrTqzA_CQFHnl
"""
...
class GradingSystem:
    ...
    def apply_hec_relative_grading(self, scores: np.array) -> List[str]:
        """
        Apply HEC relative grading based on specified ranges:
        A* : x + 2σ and above
        A  : between x + 3σ/2 and x + 2σ
        A- : between x + σ and x + 3σ/2
        B+ : between x + σ/2 and x + σ
        B  : between x - σ/2 and x + σ/2
        B- : between x - σ and x - σ/2
        C+ : between x - 4σ/3 and x - σ
        C  : between x - 5σ/3 and x - 4σ/3
        C- : between x - 2σ and x - 5σ/3
        D  : below x - 2σ
        """
        mean = np.mean(scores)
        std = np.std(scores)
    ...
    def apply_custom_relative_grading(self, scores: np.array, distribution: Dict[str, float]) -> List[str]:
        """Apply custom relative grading based on percentile distribution."""
        ...
        for i, score in enumerate(scores):
            for threshold_score, grade in score_to_grade.items():
                if score >= threshold_score:
                    grades[i] = grade
                    break
            if not grades[i]:  # If no grade assigned, give lowest grade
                grades[i] = list(distribution.keys())[-1]

        return grades

    def calculate_gpa(self, grade: str) -> float:
        """Convert letter grade to GPA according to HEC scale."""
        gpa_scale = {
    ...
    def process_grades(self, grading_method: str, custom_distribution: Dict[str, float] = None) -> pd.DataFrame:
        """Process grades for all subjects using specified grading method."""
        ...
        print("\n" + "="*80)
        print(f"GRADING RESULTS - Using {grading_method} Method")
        print("="*80)

        for subject in self.subject_columns:
            scores = self.data[subject].values
            subject_name = subject.replace('_score', '').upper()

            print(f"\n{subject_name} Analysis:")
            print("-" * 40)

            # Calculate original statistics
            orig_stats = self.calculate_statistics(scores)
            print("\nOriginal Statistics:")
            print(f"Mean: {orig_stats['mean']:.2f}")
            print(f"Standard Deviation: {orig_stats['std']:.2f}")
            print(f"Skewness: {orig_stats['skewness']:.2f}")
            print(f"Range: {orig_stats['min']:.2f} - {orig_stats['max']:.2f}")
            ...
            weighted_gpa += np.array(subject_gpas) * self.credit_hours[subject]
            ...
            for grade, count in grade_counts.items():
                ...
                print(f"{grade:2}: {count:3} students ({percentage:5.1f}%)")

            # Display GPA statistics for this subject
            subject_gpa = pd.Series(subject_gpas)
            print(f"\nGPA Statistics for {subject_name}:")
            print(f"Average GPA: {subject_gpa.mean():.2f}")
            print(f"Highest GPA: {subject_gpa.max():.2f}")
            print(f"Lowest GPA: {subject_gpa.min():.2f}")

            # Visualize
            self.visualize_distribution(
                self.apply_hec_relative_grading(scores),
                grades,
                subject.replace('_score', ''),
                grading_method
            )
        ...
        # Display overall CGPA statistics
        print("\n" + "="*80)
        print("OVERALL CGPA STATISTICS")
        print("="*80)
        print(f"\nClass Statistics:")
        print(f"Average CGPA: {results['cgpa'].mean():.2f}")
        print(f"Highest CGPA: {results['cgpa'].max():.2f}")
        print(f"Lowest CGPA: {results['cgpa'].min():.2f}")
        print(f"CGPA Standard Deviation: {results['cgpa'].std():.2f}")

        # Display CGPA distribution
        cgpa_ranges = [(4.0, "4.00"), (3.67, "3.67-3.99"), (3.34, "3.34-3.66"),
                      (3.01, "3.01-3.33"), (2.67, "2.67-3.00"), (2.34, "2.34-2.66"),
                      (2.01, "2.01-2.33"), (1.67, "1.67-2.00"), (1.31, "1.31-1.66"),
                      (1.01, "1.01-1.30"), (0.01, "0.01-1.00"), (0, "0.00")]

        print("\nCGPA Distribution:")
        print("-" * 40)
        for i in range(len(cgpa_ranges)-1):
            count = len(results[results['cgpa'].between(cgpa_ranges[i+1][0], cgpa_ranges[i][0])])
            percentage = (count/len(results)) * 100
            print(f"CGPA {cgpa_ranges[i][1]}: {count:3} students ({percentage:5.1f}%)")
        ...
        for _, student in top_5.iterrows():
            print(f"{student['first_name']} {student['last_name']}: CGPA {student['cgpa']:.2f}")

        # Save results
        results.to_csv('grading_results.csv', index=False)
        print("\nDetailed results have been saved to 'grading_results.csv'")
        print("Grade distribution visualizations have been saved as PNG files")
        logging.info("Results saved to grading_results.csv")
