...
def display_comparative_results(hec_results, custom_results, calculator):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Grade Distribution", "GPA Analysis", "Statistical Summary", "Grade Correlations"])
    ...
    with tab4:
        st.write("### HEC Grading Correlations")
        add_correlation_tab("hec", hec_results, calculator.subject_columns)
        
        if "Custom" in custom_results:
            st.write("### Custom Grading Correlations")
            add_correlation_tab("custom", custom_results, calculator.subject_columns)
...
def display_hec_results(results, calculator):
    ...
    with tab4:
        st.write("### HEC Grading Correlations")
        add_correlation_tab("hec", results, calculator.subject_columns)
        
        if "Custom" in results:
            st.write("### Custom Grading Correlations")
            add_correlation_tab("custom", results, calculator.subject_columns)
...
def display_relative_results(results, grading_system, method):
    ...
    # Create tabs for different views
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Grade Distribution", "GPA Analysis", "Statistical Summary", "Normal Distribution Analysis", "Grade Correlations"])
    ...
    with tab5:
        st.write("### HEC Grading Correlations")
        add_correlation_tab("hec", results, grading_system.subject_columns)
        
        if "Custom" in results:
            st.write("### Custom Grading Correlations")
            add_correlation_tab("custom", results, grading_system.subject_columns)
...
def create_grade_correlation_plot(results, x_subject, y_subject):
    """Helper function to create grade correlation scatter plot"""
    # Get column names
    x_score = f"{x_subject.lower()}_score"
    y_score = f"{y_subject.lower()}_score"
    x_grade = f"{x_subject.lower()}_grade"
    y_grade = f"{y_subject.lower()}_grade"
    ...
    fig = px.scatter(
        ...
        trendline="ols",
        labels={
            x_score: f"{x_subject} Score",
            y_score: f"{y_subject} Score"
        },
        title=f"Correlation between {x_subject} and {y_subject} Scores"
    )
    
    # Update traces with custom data
    fig.update_traces(
        customdata=np.column_stack((
            results[x_grade].values,
            results[y_grade].values
        )),
        hovertemplate=(
            f"{x_subject} Score: %{x}<br>" +
            f"{y_subject} Score: %{y}<br>" +
            f"{x_subject} Grade: %{customdata[0]}<br>" +
            f"{y_subject} Grade: %{customdata[1]}<br>" +
            "<extra></extra>"
        )
    )
    
    correlation = results[x_score].corr(results[y_score])
    return fig, correlation

def add_correlation_tab(tab_id, results, subject_columns):
    """Add correlation analysis to a tab"""
    st.subheader("Subject Grade Correlations")
    
    # Get subjects for selection
    subjects = [col.replace('_score', '').title() for col in subject_columns]
    
    # Create subject selection
    col1, col2 = st.columns(2)
    with col1:
        x_subject = st.selectbox(
            "Select first subject",
            subjects,
            key=f"subject1_select_{tab_id}"
        )
    
    with col2:
        remaining_subjects = [s for s in subjects if s != x_subject]
        y_subject = st.selectbox(
            "Select second subject",
            remaining_subjects,
            key=f"subject2_select_{tab_id}"
        )
    
    if x_subject and y_subject:
        try:
            fig, correlation = create_grade_correlation_plot(results, x_subject, y_subject)
            st.plotly_chart(fig, use_container_width=True)
            
            # Display correlation interpretation
            st.write(f"Correlation coefficient: {correlation:.3f}")
            if abs(correlation) >= 0.7:
                st.write("Strong correlation between the subjects")
            elif abs(correlation) >= 0.4:
                st.write("Moderate correlation between the subjects")
            else:
                st.write("Weak correlation between the subjects")
        except Exception as e:
